    ...
    <div class="card card-custom gutter-b">
        <div class="card-header border-0 py-5">
            <h3 class="card-title align-items-start flex-column">
                <span class="card-label font-weight-bolder text-dark">Alterar webhook {{ $form->getModel()->url }}</span>
            </h3>
        ...
        <div class="card-body py-0">
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <div id="formItens">
                        @isset($form->getModel()->webhookEventos)
                            @foreach($form->getModel()->webhookEventos as $item)
                                <input class="form-control" name="webhookEventos[{{ $loop->index }}][id]" type="hidden" value="{{ $item->id }}">
                                <input class="form-control" name="webhookEventos[{{ $loop->index }}][evento]" type="text" placeholder="Informe o nome do evento" value="{{ $item->evento }}">
                            @endforeach
                        @else
                            @php
