...
<div class="topbar">
    ...
    @if (config('layout.extras.quick-actions.display'))
        ...
        <div class="topbar-item">
            <div class="btn btn-icon btn-hover-transparent-white btn-dropdown btn-lg mr-1" id="kt_quick_aside_toggle">
    ...
    {{-- Settings --}}
    @if (config('layout.extras.config.display'))
        <div class="topbar-item">
            <div class="btn btn-icon btn-clean btn-lg mr-1" id="topbar-settings">
                <a href="{{ route('web.settings') }}">
                    {{ Metronic::getSVG("media/svg/icons/General/Settings-1.svg", "svg-icon-xl svg-icon-primary") }}
                </a>
            </div>
        </div>
    @endif
    ...
    @if (config('layout.extras.quick-panel.display'))
        <div class="topbar-item">
            <div class="btn btn-icon btn-lg mr-1 pulse pulse-danger" id="kt_quick_panel_toggle">
                {{ Metronic::getSVG("media/svg/icons/General/Bell.svg", "svg-icon-xl svg-icon-primary") }}
                @if(auth()->user()->unreadNotifications->count() > 0)
                    <span class="pulse-ring" id="pulse-ring-notification"></span>
                @endif
            </div>
        </div>
    @endif
        ...
        @if (config('layout.extras.user.layout') == 'offcanvas')
            <div class="topbar-item">
                <div class="btn btn-icon w-auto btn-clean d-flex align-items-center btn-lg px-2"
                    ...
                    <span class="text-muted font-weight-bold font-size-base d-none d-md-inline mr-1">{{ Auth::user()->getContaAtual()->empresa ?? '' }} • </span>
                    <span
                        class="text-primary font-weight-bolder font-size-base d-none d-md-inline mr-3">{{auth()->user()->nome}}</span>
                    <span class="symbol symbol-35 symbol-light-success">
                        <img src="{{ Helper::getAvatarUrl(auth()->user()->email, 35) }}" alt="">
                        {{-- TODO  Caso não tenha foto: --}}
                        {{--<span class="symbol-label font-size-h5 font-weight-bold">{{auth()->user()->nome[0]}}</span>--}}
                    </span>
                </div>
            </div>
        @else
            <div class="dropdown">
                ...
                <div class="topbar-item" data-toggle="dropdown" data-offset="0px,0px">
                    <div class="btn btn-icon w-auto btn-clean d-flex align-items-center btn-lg px-2">
                        ...
                        <span
                            class="text-dark-50 font-weight-bolder font-size-base d-none d-md-inline mr-3">{{auth()->user()->nome}}</span>
                        <span class="symbol symbol-35 symbol-light-success">
                            <span
                                class="symbol-label font-size-h5 font-weight-bold">{{auth()->user()->sobrenome}}</span>
                        </span>
                    </div>
                </div>
